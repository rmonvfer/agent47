 ...
 * orchestrator followed by every running/queued background agent, Claude-Code style — a state
 * dot, the agent's name, its truncated current-activity snippet, and a right-aligned elapsed time
 * and token count where the registry has them. A background agent's dot is colored by its
 * identity ([agentIdentityColor], the same color as its focus-mode header) so it stays
 * recognizable across rows; main's dot instead reflects run state (accent while streaming, muted
 * while idle), since there is only ever one of it. The highlighted row while [selectionMode] is
 * active renders like an overlay selection (filled background, bold). A leading blank line
 * separates the list from the status bar above it. Renders nothing when [rows] is empty; [rows]
 * (and therefore this panel's visibility) is the same list the layout math sizes against, so an
 * empty list here means zero reserved height too.
 */
@Composable
internal fun AgentListPanel(
    ...
    Column {
        ...
        rows.forEachIndexed { index, row ->
            ...
            Text(agentRowLine(row, nameWidth, width, selected, theme))
        }
    }
}
...
private fun agentRowLine(
    ...
    val dotColor = if (row.isMain) mainDotColor(row.state, theme) else agentIdentityColor(row.id, theme)
    ...
    // "●" + " " + name + "  " before the activity snippet, flush with the pane's own left inset
    // (no extra indent of its own) to match the status bar and the rest of the chrome.
    val prefix = "● $namePadded  "
    ...
    return buildAnnotatedString {
        withStyle(rowStyle(dotColor, bg, selected)) { append("●") }
...
private fun mainDotColor(state: AgentRowState, theme: ThemeConfig): Color =
    if (state == AgentRowState.RUNNING) theme.colors.accent else theme.colors.muted
